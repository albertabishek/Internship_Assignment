...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(DJANGO_API_URL, json=user_data, headers=headers)
        ...
        elif response.status_code == 400 and 'telegram_id' in response.json() and 'unique' in str(response.json()['telegram_id']):
        ...
        reply_text = "Sorry, the server is currently unavailable. Please try again later."
